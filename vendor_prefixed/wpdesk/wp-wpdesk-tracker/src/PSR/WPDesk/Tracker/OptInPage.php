...
namespace ShopMagicCF7Vendor\WPDesk\Tracker;

use ShopMagicCF7Vendor\WPDesk\PluginBuilder\Plugin\Hookable;
use ShopMagicCF7Vendor\WPDesk\View\Renderer\SimplePhpRenderer;
use ShopMagicCF7Vendor\WPDesk\View\Resolver\DirResolver;
class OptInPage implements \ShopMagicCF7Vendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function __construct($plugin_file, $plugin_slug, $shop = null)
    {
        ...
        $this->shop = $shop ?? new \ShopMagicCF7Vendor\WPDesk\Tracker\Shop('');
    }
    ...
    public function output()
    {
        ...
        if (isset($_GET['shop_url'])) {
            $shop = new \ShopMagicCF7Vendor\WPDesk\Tracker\Shop(\sanitize_text_field(\wp_unslash($_GET['shop_url'])));
        } else {
            $shop = $this->shop;
        }
        ...
        $renderer = new \ShopMagicCF7Vendor\WPDesk\View\Renderer\SimplePhpRenderer(new \ShopMagicCF7Vendor\WPDesk\View\Resolver\DirResolver(__DIR__ . '/views'));
    ...
    public function admin_init()
    {
        if (isset($_GET['wpdesk_tracker']) && $_GET['wpdesk_tracker'] === $this->plugin_slug) {
            if (isset($_GET['allow']) && isset($_GET['security']) && \wp_verify_nonce($_GET['security'], $this->plugin_slug)) {
                if ($_GET['allow'] === '1') {
                    $persistence = new \ShopMagicCF7Vendor\WPDesk_Tracker_Persistence_Consent();
